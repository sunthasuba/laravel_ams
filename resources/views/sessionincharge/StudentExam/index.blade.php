                ...
                <div class="card border-0 shadow-lg">
                    <div class="card-header border-0 bg-transparent mb-2 fs-2 text-primary lead">
                        <div class="d-flex justify-content-between">
                            <span>{{ __('Student Attendance Table') }}</span>
                            ...
                            <button type="button" class="btn btn-primary rounded-0 shadow-none ms-auto"
                                ...
                                {{ __('Add Attendance Student Table') }}
                            </button>
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <button type="button"
                                                class="addExamStudent rounded-0 shadow-none btn btn-primary">{{ __('Add Student Course') }}</button>
                                            <button type="button"
                                                class="updateExamStudent rounded-0 shadow-none btn btn-info">{{ __('Update Student Course') }}</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.addExamStudent', function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "/sessionincharge/studentExam/store",
                    data: data,
                    dataType: "json",
                    success: function(response) {
            ...
            $(document).on('click', '.studentExams-edit', function(e) {
                ...
                $.ajax({
                    type: "GET",
                    url: `/sessionincharge/studentExam/edit/${examId}`,
                    dataType: "json",
                    success: function(response) {
            ...
            $(document).on('click', '.updateExamStudent', function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "/sessionincharge/studentExam/update",
                    data: data,
                    dataType: "json",
                    success: function(response) {
                        $('.exam_id').removeClass('is-invalid');
            ...
            $(document).on('click', '.studentExams-delete', function(e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: "GET",
                            url: `/sessionincharge/studentExam/destroy/${Id}`,
                            dataType: "json",
                            success: function(response) {
                                fetch();
                                if (response.status == true) {
